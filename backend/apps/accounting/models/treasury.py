...
class CapitalSource(models.Model):
    """
    Represents where the money came from. Linked to an EQUITY or LIABILITY GL Account.
    """
...
class CashTransfer(models.Model):
    """
    Moving cash between bank/cash accounts.
    Now points directly to Account (ASSET type).
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    date = models.DateField()
    from_bank = models.ForeignKey(Account, on_delete=models.PROTECT, related_name="transfers_out")
    to_bank = models.ForeignKey(Account, on_delete=models.PROTECT, related_name="transfers_in")
