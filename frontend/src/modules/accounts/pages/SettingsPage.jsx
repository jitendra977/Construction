/**
 * SettingsPage — Admin / user settings hub.
 *
 * Sections
 * ────────────────────────────────────────────────────────
 *  1. Appearance       — dark/light theme, UI density
 *  2. Notifications    — email alerts, sound, in-app banners
 *  3. Language         — UI language + date/currency locale
 *  4. Security         — active sessions, 2-FA status (read-only)
 *  5. System Info      — API URL, env, Sentry status, version
 *  6. Data & Cache     — export personal data, clear local cache
 *
...
function AppearanceSection() {
    ...
    const handleDensity = (val) => {
        setDensity(val);
        localStorage.setItem('ui-density', val);
    };
        ...
        <div style={card}>
            ...
            <div style={rowLast}>
                <div>
                    ...
                    <p style={rowSub}>Primary colour used for buttons and highlights.</p>
                </div>
...
function NotificationsSection({ user, updateProfile }) {
    ...
    const handleToggle = async (key, val) => {
        ...
        // Persist localStorage-only prefs immediately
    ...
    const ITEMS = [
        {
            ...
            sub: 'Task updates, approvals, and system alerts inside the app.',
            backend: true,
        },
        {
            key: 'email_alerts',
            label: 'Email Alerts',
            sub: 'Receive digest emails for approvals and critical changes.',
        },
        {
            key: 'sound_enabled',
            label: 'Notification Sounds',
            sub: 'Play a chime when a new alert arrives.',
        },
        {
            key: 'banner_alerts',
            label: 'Banner Notifications',
            sub: 'Show slide-in banners for real-time updates.',
        },
    ];
...
function LanguageSection({ user, updateProfile }) {
    ...
    const handleDateFmt = (v) => { setDateFmt(v); localStorage.setItem('date-format', v); };
    const handleCurrency = (v) => { setCurrency(v); localStorage.setItem('currency', v); };
        ...
        <div style={card}>
            ...
            <div style={row}>
                <div>
                    ...
                    <p style={rowSub}>Saved to your profile and applied on next login.</p>
                </div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <div style={row}>
                ...
                <select value={dateFmt} onChange={e => handleDateFmt(e.target.value)} style={inputStyle}>
            ...
            <div style={rowLast}>
                ...
                <select value={currency} onChange={e => handleCurrency(e.target.value)} style={inputStyle}>
...
function SecuritySection({ user }) {
    ...
    const loadSessions = useCallback(async () => {
        ...
        try {
            ...
            // Deduplicate by IP — treat each unique IP in last 30 days as an "active" session proxy
            const seen = new Set();
            const deduped = logs.filter(l => {
                if (!l.ip_address || seen.has(l.ip_address)) return false;
                seen.add(l.ip_address);
                return true;
            }).slice(0, 5);
            setSessions(deduped);
        } catch {
    ...
    return (
        <div style={card}>
            ...
            <p style={sectionSub}>Recent logins and security settings for your account.</p>
            ...
            <div style={{ ...rowLast, flexDirection: 'column', alignItems: 'flex-start', gap: 12 }}>
                ...
                {!loading && sessions && sessions.length === 0 && (
                    <p style={{ fontSize: 12, color: 'var(--t-text3)' }}>No login history found.</p>
                )}
                {!loading && sessions && sessions.length > 0 && (
                        ...
                        {sessions.map((s, i) => (
                            <div key={i} style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                    ...
                                    <div>
                                        <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: 'var(--t-text)', fontFamily: 'monospace' }}>
...
function SystemInfoSection() {
    ...
    const INFO_ROWS = [
        ...
        { label: 'Frontend Version',   value: __APP_VERSION__ ?? '—',        mono: true  },
...
function DataSection({ user }) {
    ...
    const handleClearCache = () => {
        const toRemove = Object.keys(localStorage).filter(k => !KEEP_KEYS.includes(k));
    ...
    const handleExport = () => {
        ...
        a.href     = url;
        a.download = `constructpro-profile-${new Date().toISOString().slice(0, 10)}.json`;
        a.click();
    ...
    return (
        <div style={card}>
            ...
            <p style={sectionSub}>Manage your local data and export your account information.</p>
            ...
            <div style={rowLast}>
                <div>
                    ...
                    <p style={rowSub}>Removes UI preferences and cached filters. Your login session and theme are kept.</p>
                </div>
                <button onClick={handleClearCache}
                    style={{ padding: '7px 16px', borderRadius: 10, background: cleared ? '#10b98118' : '#ef444418', color: cleared ? '#10b981' : '#ef4444', fontWeight: 800, fontSize: 12, border: `1px solid ${cleared ? '#10b98130' : '#ef444430'}`, cursor: 'pointer', transition: 'all 0.3s' }}>
    ...
    return (
        <div style={{ maxWidth: 840, margin: '0 auto' }}>
            ...
            <div style={{ marginBottom: 24 }}>
                ...
                <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--t-text3)' }}>
                    Manage your account preferences, appearance, and system configuration.
                </p>
            </div>
                            ...
                            {SECTION_TABS.map(({ key, label }) => (
                                <button key={key} onClick={() => setActiveSection(key)}
                                    style={{
                                        ...
                                        border: 'none', borderLeft: activeSection === key ? '3px solid #6366f1' : '3px solid transparent',
