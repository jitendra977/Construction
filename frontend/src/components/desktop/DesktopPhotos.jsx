            ...
            <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-700 shadow-xl pb-16 pt-8 px-6 shrink-0 relative overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto relative z-10">
                            ...
                            {tabs.map((tab) => (
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-[10px] md:text-xs font-bold uppercase tracking-wider transition-all duration-300 ${viewMode === tab.id
                                        ? 'bg-[var(--t-surface)] text-emerald-900 shadow-md transform scale-100'
                                        : 'text-white/70 hover:text-white hover:bg-[var(--t-surface)]/10'
                                        }`}
                                >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-[var(--t-surface)]/10 backdrop-blur-sm rounded-xl p-3 border border-white/20">
                        ...
                        {stats.slice(0, 3).map((stat, idx) => (
                            <div key={idx} className="bg-[var(--t-surface)]/10 backdrop-blur-sm rounded-xl p-3 border border-white/20 hidden sm:block">
                        ...
                        <div className="space-y-8 animate-fadeIn">
                            ...
                            {groupedGallery.map((group) => (
                                <div key={group.groupName} className="bg-[var(--t-surface)] rounded-3xl shadow-sm border border-[var(--t-border)] overflow-hidden">
                                    {/* Section Header */}
                                    <div className="px-6 py-4 bg-gray-50/50 border-b border-[var(--t-border)] flex items-center gap-4">
                                            ...
                                            <div className="masonry-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2">
                                                {group.items.map((item) => (
                                                    <div
                                                        ...
                                                        className="group relative cursor-pointer overflow-hidden rounded-lg aspect-square bg-gray-200 border border-[var(--t-border)]"
                                                    >
                                                        {item.type === 'VIDEO' ? (
                                            ...
                                            <div className="grid grid-cols-1 gap-3">
                                                {group.items.map((item) => (
                                                    <div key={item.id} className="flex items-center p-4 bg-[var(--t-surface2)] rounded-xl border border-[var(--t-border)] hover:bg-[var(--t-surface)] hover:border-[var(--t-border)] hover:shadow-md transition-all group cursor-pointer" onClick={() => window.open(item.url, '_blank')}>
                                                        <div className="w-12 h-12 rounded-lg bg-red-100 text-[var(--t-danger)] flex items-center justify-center text-2xl mr-4 shrink-0">
                                                            {item.type === 'PDF' ? '📄' : '🖼️'}
                                                        </div>
